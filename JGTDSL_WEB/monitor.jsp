...
<%@ page import="net.sf.ehcache.management.CacheStatistics" %>
<%@page import="net.sf.ehcache.statistics.StatisticsGateway"%>
...
<form method="post" name="clearCacheForm" action="monitor.jsp">
...
<%
  ...
  for (int i = 0; i < cacheNames.length; i++) {
	 ...
	 cacheHits += statisticsGateway.cacheHitCount();
	 cacheMisses += statisticsGateway.cacheMissCount();
	 //inMemoryHits += statisticsGateway.cachein .cache.getInMemoryHits();
	 //objectCount += statisticsGateway.cache .getObjectCount();
	 //onDiskHits += statisticsGateway.cached();
%>
        <tr>
	        <td> <A href=monitor.jsp?showCache=<%=statisticsGateway.getAssociatedCacheName() %>> <%=statisticsGateway.getAssociatedCacheName() %></A></td>
	        <td> <%=statisticsGateway.cacheHitCount() %> </td>
	        <td> <%=statisticsGateway.cacheMissCount() %> </td>
	        <td>
